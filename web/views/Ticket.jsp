      ...
      <div class="main-panel">          
        <div>
          <div class="col-lg-10 stretch-card">
            <div class="card">
              <div class="card-body">
                <h4 class="card-title">Les tickets ouverts</h4>
...
<!--          <input type="text" class="form-control" placeholder="Rechercher une demande" ng-model="recherche">-->
        ...
        <%
            ResultSet data=MyConnection.getMyConnection().createStatement().executeQuery("select * from TICKETAFFECTÉES join intervenant"
                + " on(TICKETAFFECTÉES.id_intervenant=intervenant.id_intervenant) where intervenant.ID_INTERVENANT="+id);
        %>
        ...
        %>
                        <%while(data.next()){%>  
                          <tr ng-repeat = "mail in mails">
                            ...
                            <td><%=data.getString("DATE_CREATION_TICKET")%></td>
                            <td>
                              <!--<button type="button" class="btn btn-sm btn-secondary">Cloturer</button>-->
                              <a class="btn btn-sm btn-secondary" href="solutionTicket.jsp?idticketaffecte=<%=data.getInt("ID_TICKETAFFECT")%>&nomemploye=<%=data.getString("NOM_EMPLOYE")%>&prenomemploye=<%=data.getString("PRENOM_EMPLOYE")%>&telephone=<%=data.getString("TELEPHONE")%>&typeprobleme=<%=data.getString("TYPE_PROBLEME_TICKET")%>&description=<%=data.getString("DESCRIPTION")%>&datecreation=<%=data.getDate("DATE_CREATION_TICKET")%>">ouvrir</a>
                            </td>
                            <!--<td>-->
