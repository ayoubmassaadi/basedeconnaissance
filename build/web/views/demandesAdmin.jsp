...
<!--          <input type="text" class="form-control" placeholder="Rechercher une demande" ng-model="recherche">
          <div class="input-group-btn">
            ...
            <table class="table table-hover">
                <%
                    ResultSet data=MyConnection.getMyConnection().createStatement().executeQuery("select * from demande "
                            + "join employee on(demande.id_employe=employee.id_employe)");
//                    ResultSet data=MyConnection.getMyConnection().createStatement().executeQuery("select * from demande "
//                            + "join employee on(employee.id_employe=demande.id_employe) join HISTORIQUEDEM2 on(employee.id_employe=HISTORIQUEDEM2.id_employe)");
...
<!--                        <th> Id employé</th>-->
                        ...
                        <tbody>    
                          <%while(data.next()){%>  
                        <form action="../DemandesAdmin" method="GET">  
                          <tr>
                            <td hidden><input readonly  type="text" name="idDemande" value="<%=data.getInt("id_demande")%>"></td> 
                            ...
                            <td><input readonly type="text" name="typeprobleme" value="<%=data.getString("TYPE_PROBLEME_DEMANDE")%>"></td>
<!--                            <td><input readonly type="text" name="description" value="<%=data.getString("DESCRIPTION")%>"></td>-->
                            ...
                            <td><input readonly type="text" name="datecreationdemande" value="<%=data.getDate("DATE_CREATION_DEMANDE")%>"></td>
                            <td>
                            ...
                            <td>
                              ...
                              <!--<a class="btn btn-sm btn-secondary" href="ajoutticket.jsp?idEmploye=<%=data.getInt("ID_EMPLOYE")%>&nomemploye=<%=data.getString("NOM_EMPLOYE")%>&prenomemploye=<%=data.getString("PRENOM_EMPLOYE")%>&emailemploye=<%=data.getString("EMAIL_EMPLOYE")%>&telephone=<%=data.getString("TELEPHONE")%>&typeprobleme=<%=data.getString("TYPE_PROBLEME_DEMANDE")%>&description=<%=data.getString("DESCRIPTION")%>">creer un ticket</a>-->
                            </td>
                          </tr>
                          <%}%>
                        </tbody>
